	...
	while (is_turk_cost_valid(turk_cost, i) == true && item != NULL)
	{
		if ((target_a >= item->value && (target_a < turk_cost->target_a[i]
			...
			|| (target_a <= turk_cost->target_a[i]
				&& turk_cost->target_a[i] <= item->value))
		{
			turk_cost->rotate_cost[i] = rotate_cost;
...
// time : O(n)
// space: O(n)
void	rrotate_turk_cost(t_turk_costs *dst, size_t rotate_b, size_t rotate_a, size_t length_a)
{
	int	*temp;
...
void	rotate_turk_cost(t_turk_costs *dst, size_t rotate_b, size_t rotate_a, size_t length_a)
{
	...
	if (is_turk_cost_valid(dst, 0) == true)
	{
		...
		free(dst->target_a);
		dst->target_a = temp;
		rotate_value(dst->rotate_cost, dst->length, rotate_a, length_a);
	}
}
...
void	double_draw(t_2intlist *src, t_turk_costs *turk_cost, int show)
{
		...
		&& is_turk_cost_valid(turk_cost, 0) == true)
	{
		...
		// write(1, "] = ", 4);
		// ft_putnbr_fd(turk_cost->target_a[cheap_trick], 1, "0123456789", 1);
		// write(1, "\n", 1);
		write(1, "rotate_cost[", 13);
		ft_putnbr_fd(cheap_trick, 1, "0123456789", 1);
		write(1, "] = ", 4);
		ft_putnbr_fd(rotate_effect, 1, "0123456789", 1);
		write(1, "\n", 1);
		// write(1, "src->a.item_1st = ", 19);
		// ft_putnbr_fd(src->a.item_1st->value, 1, "0123456789", 1);
		// write(1, "\n", 1);
		// write(1, "src->a.item_1st->next = ", 25);
		// ft_putnbr_fd(src->a.item_1st->next->value, 1, "0123456789", 1);
		// write(1, "\n", 1);
		// write(1, "src->a.item_last = ", 20);
		// ft_putnbr_fd(src->a.item_last->value, 1, "0123456789", 1);
		// write(1, "\n", 1);
		...
		if (rotate_effect < 0)
			rrotate_turk_cost(turk_cost, cheap_trick, (size_t)(-1 * rotate_effect) + 1, src->a.length - 1);
		else
			rrotate_turk_cost(turk_cost, cheap_trick, (size_t)rotate_effect, src->a.length - 1);
		write(1, "&&&", 3);
